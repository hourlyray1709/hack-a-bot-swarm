...
TURN_IN_PLACE_RAD = 0.35   # start rotate-only if error > 20 deg
DRIVE_RAD         = 0.17   # allow forward once error < 10 deg
ARRIVE_THRESH_M   = 0.06
...
class SwarmCoordinator:
    ...
    def _drive_to(bot, target):
        ...
        heading_err = (target_heading - bot.heading + math.pi) % (2 * math.pi) - math.pi

        ALIGN_MOVE = 0.20   # ~11.5 deg: must be within this to go straight

        # rotate only until aligned enough
        if abs(heading_err) > ALIGN_MOVE:
            turn_pwm = int((abs(heading_err) / math.pi) * MAX_SPEED)
            turn_pwm = max(MIN_SPEED, min(MAX_SPEED, turn_pwm))

            # if it turns wrong way, swap signs below
            if heading_err > 0:
                return BotCommand(left=-turn_pwm, right=turn_pwm)   # CCW
            else:
                return BotCommand(left=turn_pwm, right=-turn_pwm)   # CW

        # then straight only
        fwd = int(KP_LINEAR * dist * MAX_SPEED)
        fwd = max(MIN_SPEED, min(MAX_SPEED, fwd))
        return BotCommand(left=fwd, right=fwd)
    ...
    def _guard_target(trolley: tuple[float, float], obstacles: list[tuple[float, float]]) -> tuple[float, float]:
        ...
        if threats:
            ...
            target_obs = min(threats, key=lambda o: abs(
                o[1] - ARENA_HEIGHT_M / 2))
